...
def random_5(array)
  ...
  var_rand = Random.new
  5.times do
    array_rand << array[var_rand.rand(0...array.size)]
  end
  return array_rand
end

# pour envoyer des tweets à 5 journalistes au hasard
# send_tweet(random_5(journalists), "Bonjour, bravo pour votre dernier article ! #bonjour_monde @the_hacking_pro")

############################################
## 2.2. Il va liker bonjour
############################################
def like_tweets_hashtag(my_hashtag, number)
  login_twitter.search(my_hashtag, result_type: "recent").take(number).collect do |tweet|
    login_twitter.favorite(tweet)
  end
end

# like_tweets_hashtag("#bonjour_monde", 25)

############################################
## 2.3. Il va follow bonjour
############################################
def follow_tweets_hashtag(my_hashtag, number)
  login_twitter.search(my_hashtag, result_type: "recent").take(number).collect do |tweet|
